...
export default function GoalFormDrawer({
  ...
  const categoryOptions = useMemo(() => {
    const names = categories
      ...
      .filter(Boolean);
    return [...new Set(["Personal", ...names, reviewValues.category].filter(Boolean))];
  ...
  const footer = (
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 1.5 }}>
      ...
      {isReviewStep ? (
        ...
        <Button type="button" variant="contained" onClick={handleContinueToReview}>
          Next: Review
        </Button>
      )}
    </Box>
  ...
  const selectedCategoryMeta = categoryMetaByName.get(reviewValues.category?.trim().toLowerCase());
  const selectedIconMeta =
    CATEGORY_ICON_OPTIONS.find((item) => item.value === selectedCategoryMeta?.icon) ||
  ...
  return (
    <AppDrawer
      ...
      onClose={onClose}
      title={title}
      subtitle={isReviewStep ? `${modeLabel} - Review` : "Capture category, amount, date, and notes"}
      footer={footer}
    >
      <Box
        sx={{
          ...
          >
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Please review your goal details before {drawerMode === "edit" ? "updating" : "creating"}.
              </Typography>
            </Box>
            <Divider />
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Typography variant="caption" sx={{ width: "100px" }} color="text.secondary">Goal Name</Typography>
              <Typography variant="body1" sx={{ fontWeight: 400, textAlign: "left" }}>{reviewValues.name || "-"}</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Typography variant="caption" sx={{ width: "100px" }} color="text.secondary">Goal Category</Typography>
              <Typography variant="body1" sx={{ fontWeight: 400, textAlign: "left" }}>{reviewValues.category || "-"}</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Typography variant="caption" sx={{ width: "100px" }} color="text.secondary">Target Amount</Typography>
              <Typography variant="body1" sx={{ fontWeight: 400, textAlign: "left" }}>
                ₹{Number(reviewValues.targetAmount || 0).toLocaleString()}
              </Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Typography variant="caption"sx={{ width: "100px" }}  color="text.secondary">Target Date</Typography>
              <Typography variant="body1" sx={{ fontWeight: 400, textAlign: "left" }}>
                {reviewValues.deadline ? dayjs(reviewValues.deadline).format("DD MMM YYYY") : "Not set"}
              </Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "flex-start", gap: 2 }}>
              <Typography variant="caption" sx={{ width: "100px" }} color="text.secondary">Description</Typography>
              <Typography variant="body1" sx={{ fontWeight: 400, textAlign: "left", maxWidth: "65%" }}>{reviewValues.description || "Not added"}</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Typography variant="caption" sx={{ width: "100px" }} color="text.secondary">Initial Amount</Typography>
              <Typography variant="body1" sx={{ fontWeight: 400, textAlign: "left" }}>
