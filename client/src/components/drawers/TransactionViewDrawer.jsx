...
import { SectionCard, StatusChip } from "../common";
import AppDrawer from "./AppDrawer";

const formatCurrency = (value) => `₹${Number(value || 0).toLocaleString()}`;

export default function TransactionViewDrawer({
  open,
  onClose,
  transaction,
  goalName,
    ...
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 1.5 }}>
      ...
      {transaction ? (
        <Box sx={{ display: "flex", gap: 1 }}>
          {onDelete ? (
            <Button type="button" color="error" variant="outlined" onClick={() => onDelete(transaction)}>
              Delete
            </Button>
          ) : null}
          {onEdit ? (
            <Button type="button" variant="contained" onClick={() => onEdit(transaction)}>
              Edit Transaction
            </Button>
          ) : null}
        </Box>
      ...
      {!transaction ? (
        ...
        <Stack spacing={2}>
            ...
            >
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 1.5 }}>
                <Box>
                  ...
                  <Typography variant="h4" sx={{ mt: 0.5, fontWeight: 800, color: `${amountTone}.main` }}>
                    {transaction.type === "expense" ? "-" : "+"} {formatCurrency(transaction.amount)}
                  </Typography>
                </Box>
                <StatusChip label={transaction.type || "Unknown"} tone={amountTone} />
              </Box>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 1.5 }}>
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary">Bank / Source</Typography>
                  <Typography variant="body1" sx={{ mt: 0.5, fontWeight: 600 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body1" sx={{ mt: 0.5, fontWeight: 600 }}>
                    {new Date(transaction.date || transaction.createdAt || Date.now()).toLocaleDateString()}
                  </Typography>
                </Box>
                <Box>
          ...
          <SectionCard title="Notes">
            <Typography variant="body2" color={transaction.notes ? "text.primary" : "text.secondary"}>
