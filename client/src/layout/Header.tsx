...
import LabeledDateField from "../components/common/LabeledDateField";
...
export default function Header({ activeScreen, config }: HeaderProps) {
  ...
  const setScopeMode = usePageDateFilterStore((state) => state.setScopeMode);
  ...
  const setSelectedYear = usePageDateFilterStore(
    (state) => state.setSelectedYear,
  );
  const setSelectedMonth = usePageDateFilterStore(
  ...
  const setRangeStart = usePageDateFilterStore((state) => state.setRangeStart);
  const setRangeEnd = usePageDateFilterStore((state) => state.setRangeEnd);

  const currentFiscalYearStart = getFiscalYearStart(new Date());
  const availableYears = Array.from(
    { length: 4 },
    (_, index) => currentFiscalYearStart - index,
  );
  const supportsHeaderDateFilter =
    activeScreen === "dashboard" ||
    activeScreen === "transactions" ||
    activeScreen === "categories";
  const showHeaderDateFilter =
    supportsHeaderDateFilter && (config?.showHeaderDateFilter ?? true);
  const isDateFilterOpen = Boolean(dateFilterAnchorEl);

  const dateFilterSummary = useMemo(() => {
    if (scopeMode === "tillNow") return "Till Now";

    if (scopeMode === "range") {
      if (!rangeStart || !rangeEnd) return "Pick a range";
      return `${dayjs(rangeStart).format("D MMM")} – ${dayjs(rangeEnd).format("D MMM")}`;
    }
  ...
  }, [scopeMode, periodMode, selectedMonth, selectedYear, rangeStart, rangeEnd]);

  const handleScopeModeChange = (
    _event: React.MouseEvent<HTMLElement>,
    value: GlobalDateScopeMode | null,
  ) => {
            ...
            <>
              ...
              >
                <AppButton
                  ...
                  sx={{
                    ...
                    backgroundColor: isDateFilterOpen
                      ? "action.hover"
                      : "transparent",
                  }}
                >
                ...
                <Box sx={{ minWidth: 0 }}>
                  ...
                  >
                    {scopeMode === "tillNow" ? "Showing" : "Active Filter"}
                  </Typography>
                  <Typography
                    variant="body2"
              ...
              >
                <Box sx={{ display: "grid", gap: 1.25 }}>
                  ...
                  >
                    <ToggleButton value="tillNow">Till Now</ToggleButton>
                    <ToggleButton value="fiscal">Fiscal</ToggleButton>
                      ...
                      {periodMode === "monthly" ? (
                        ...
                        >
                          {FISCAL_MONTH_ORDER.map((monthIndex) => (
                            <MenuItem
                              key={PAGE_MONTH_OPTIONS[monthIndex]}
                              value={monthIndex}
                            >
                      ...
                      >
                        {availableYears.map((year) => (
                          <MenuItem
                            key={year}
                            value={year}
                          >{`FY ${year}-${String(year + 1).slice(-2)}`}</MenuItem>
                        ))}
                      </Select>
                    </>
                  ) : null}
                    ...
                    <Box sx={{ display: "grid", gap: 1 }}>
                      <LabeledDateField
                        ...
                        onChange={(value: Dayjs | null) =>
                          setRangeStart(value ? value.format("YYYY-MM-DD") : null)
                        }
                      />
                      <LabeledDateField
                        labelText="To"
                        ...
                        onChange={(value: Dayjs | null) =>
                          setRangeEnd(value ? value.format("YYYY-MM-DD") : null)
                        }
                      />
                    </Box>
                  ) : null}
                  ...
                  {scopeMode === "tillNow" ? (
                    <Typography variant="caption" color="text.secondary">
                      Showing all-time data, no date restriction.
                    </Typography>
                  ) : null}
                </Box>
              </Popover>
