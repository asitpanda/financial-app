...
import { validateAssetTaxonomyForm } from "../investment.schema";
...
function TaxonomyTreeItemLabel({
  children,
  node,
  isSelected,
  onEditNode,
...
const TaxonomyTreeItem = React.forwardRef(
  function TaxonomyTreeItem(props, ref) {
    const {
      onEditNode,
      onAddChildNode,
      onDeleteNode,
...
export default function InvestmentAssetTaxonomyFormDrawer({
  ...
  const selectedLineage = useMemo(
    () => buildLineage(selectedNode, taxonomyNodesById),
  ...
  const taxonomyTreeVersion = useMemo(
    () =>
      sortedTaxonomyNodes
        .map((node) => `${node.id}:${node.parentId ?? "root"}:${node.level}`)
        .join("|"),
    [sortedTaxonomyNodes],
  );
  ...
  useEffect(() => {
    ...
    const firstNodeId = sortedTaxonomyNodes[0]
      ? String(sortedTaxonomyNodes[0].id)
      : null;
  ...
  useEffect(() => {
    ...
    setExpandedItems((current) =>
      current.filter((itemId) => validNodeIds.has(String(itemId))),
    );
  ...
  const handleDeleteNode = () => {
    ...
    if (String(selectedNodeId) === String(deleteTarget.id)) {
      setSelectedNodeId(
        deleteTarget.parentId ? String(deleteTarget.parentId) : null,
      );
    ...
    <AppDrawer
      ...
      subtitle={
        selectedNode
          ...
          : "Create and maintain the asset hierarchy used by the investment drawer."
      }
      width={1040}
      footer={footer}
    >
          ...
          {taxonomyTree.length ? (
            <Paper
              ...
              sx={{
                ...
                overflow: "hidden",
                background: (theme) =>
                  `linear-gradient(180deg, ${alpha(theme.palette.primary.main, 0.04)} 0%, ${theme.palette.background.paper} 100%)`,
              }}
            >
              <RichTreeView
                ...
                slotProps={{
                  item: {
                    ...
                    onAddChildNode: (node) => {
                      ...
                      setErrors({});
                      setExpandedItems((current) =>
                        Array.from(new Set([...current, String(node.id)])),
                      );
                    },
                    onDeleteNode: (node) => {
                      setSelectedNodeId(String(node.id));
            ...
            <Typography variant="body2" color="text.secondary">
              No asset taxonomy exists yet. Start with a root category from the
            ...
            <Stack spacing={2}>
              <Paper
                ...
                sx={{
                  ...
                  borderRadius: 1,
                  background: (theme) =>
                    `linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.07)} 0%, ${alpha(theme.palette.success.main, 0.05)} 100%)`,
                }}
              >
                <Box
            ...
            <Stack spacing={2}>
              ...
              >
                ...
                <LabeledTextField
                  ...
                  value={form.nodeType}
                  onChange={(event) =>
                    handleFormChange("nodeType", event.target.value)
                  }
                ...
                <LabeledSelectField
                  ...
                  value={form.parentId}
                  onChange={(event) =>
                    handleFormChange("parentId", event.target.value)
                  }
                ...
                <LabeledTextField
                  ...
                  onChange={(event) =>
                    handleFormChange("sortOrder", event.target.value)
                  }
                ...
                <LabeledSelectField
                  ...
                  value={form.isActive}
                  onChange={(event) =>
                    handleFormChange("isActive", event.target.value)
                  }
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `Remove ${deleteTarget.label} from the asset taxonomy? Child nodes under this branch will also be removed.`
            : ""
        }
        confirmLabel="Delete"
        confirmColor="error"
